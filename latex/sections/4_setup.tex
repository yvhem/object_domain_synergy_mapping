...
The position variation error $e_\text{pos}(t)$ is then obtained as the magnitude of the difference between the human and robot relative displacements:
\begin{equation}
    e_\text{pos}(t) = ||\Delta \bs{p}_H(t) - \Delta \bs{p}_R(t)||
\end{equation}

\paragraph{Rotation variation error} This metric evaluates the tracking of the sphere's orientation. Let $\bs{q}(t)$ be the quaternion representing the sphere's rotation. The relative rotation from $t_0$ is computed as:
\[ \bs{q}_\text{rel}(t) = \bs{q}(t) \otimes \bs{q}(t_0)^{-1} \]
where $\otimes$ denotes quaternion multiplication and $^{-1}$ denotes the quaternion conjugate.

We define an extraction operator $\Phi(\bs{q})$ that returns the rotation angle (magnitude) from a quaternion:
\[ \Phi(\bs{q}) = 2\arccos(q_w) \]
...
\begin{equation}
    \bs{q}_\text{diff}(t) = \bs{q}_{R, \text{rel}}(t) \otimes \bs{q}_{H, \text{rel}}(t)^{-1} \quad \Rightarrow \quad e_\text{rot}(t) = \Phi(\bs{q}_\text{diff}(t))
\end{equation}

\subsection{Energy metrics}
Besides the geometry of the virtual sphere, we are also interested in evaluating the \textit{intensity} of the grasp. In the virtual sphere framework, the squeezing of the object corresponds to an accumulation of elastic potential energy in the virtual stiffness model.

\paragraph{Elastic energy error} $R_H^{(t)}$ and $R_R^{(t)}$ be the human and robotic hand virtual sphere radius, respectively, at a generic time $t$. The deformation is then computed as the normalized difference between the current radius and the initial one:
\begin{equation}
    \Delta_{H/R} = \frac{\max(0, R_{H/R}^0 - R_{H/R}^{(t)})}{R_{H/R}^0}
\end{equation}
...
\begin{equation}
    U_{H/R}(t) = \frac{1}{2} k_{cp} k_s (\Delta_{H/R}(t))^2
\end{equation}
...
\[ e_{\%, \text{smooth}}(t) = \mathcal{S}(e_{\%}(t)) \]
