...
In their study, Santello et al. \cite{santello1998postural} analyzed the hand postures of different subjects while grasping imaginary objects. By applying \textit{Principal Component Analysis} (PCA) to the collected joint angle data, they identified that a large portion of the variance in hand postures could be explained by a very small number of principal components. Specifically:
\begin{itemize}
    \item the first principal components (\textit{synergies}) account for more than 80\% of the variance in hand posture;
    \item the first synergy corresponds to the coordinated flexion and extension of all fingers (opening and closing the hand), resembling a power grasp *TODO: add figure*;
    \item the second synergy accounts for the abduction of the fingers and the opposition of the thumb, effectively controlling the arching of the palm *TODO: add figure*;
    \item the remaining synergies can be used to fine-tune the hand posture for specific grasp types, but contribute increasingly less to the overall variance.
\end{itemize}
